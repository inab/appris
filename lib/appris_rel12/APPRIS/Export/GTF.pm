...
$GTF_CONSTANTS = {
	...
	'cexonic'=>{
		'source'=>'CEXONIC',
		'type'=>'exon_conservation',
		'annot'=>['exon_conservation','non-aligned_introns'],
		#'annot'=>['Exonic structure conserved in Mouse','Exonic structure not conserved in Mouse'],
	},
};
		...
		if ($feature->stable_id) {
			...
			if ($feature->translate and $feature->translate->sequence) { # proteing coding methods
				...
				if ( ($source =~ /cexonic/) or ($source eq 'all') ) {
					$output .= get_cexonic_annotations(	$transcript_id,
	   		        										$gene_id,
	   		        										$external_id,
	           												$feature,
	           												$version
				...
				#	$output .= APPRIS::Export::PROTEO::get_proteo_annotations(	$transcript_id,
				#		   		        										$gene_id,
				#		   		        										$external_id,
				#		           												$feature,
				#		           												$version
			...
			if ( defined $method->residues ) {
				foreach my $region (@{$method->residues}) {
					if ( defined $region->residue and 
						...
						# optinal attributes
						my($optional);
						$optional->{'note'}				= "pep_position:".$region->residue;
						$optional->{'gene_id'}			= $gene_id;
						$optional->{'transcript_id'}	= $transcript_id;
						$optional->{'transcript_name'}	= $external_id;
						$optional->{'version'}			= $version;		
...
=head2 get_cexonic_annotations
  ...
  Arg [4]    : APPRIS::Transcript
  Arg [5]    : String - $version
  Example    : $annot = get_cexonic_annotations($trans_id, $gen_id, $ext_id, $feat, $v);
  Description: Retrieves specific annotation.
  Returntype : String or undef
	...
	# Get annotations
 	if ( $feature->analysis ) {
			...
			if ( defined $method->regions ) {
				foreach my $region (@{$method->regions}) {
					if ( defined $region->start and defined $region->end and defined $region->strand ) {
						# get type
						my ($method_type) = $GTF_CONSTANTS->{'cexonic'}->{'annot'}->[1];	
						# common attributes
						my ($common) = {
								'seqname'	=> $feature->chromosome,
								'source'	=> $method_source,
		...
		# get appris annotations 		
 		if ( $analysis->appris ) {
 			my ($method) = $analysis->appris;
			if ( defined $method->exon_conservation_signal ) {				
				my ($annot) = $method->exon_conservation_signal;
				my ($method_type) = $GTF_CONSTANTS->{'cexonic'}->{'type'};
				$output .= get_appris_annotations2(	$transcript_id,
													$gene_id,
													$external_id,
													$feature,
		...
		# get appris annotations 		
 		if ( $analysis->appris ) {
			...
			if ( defined $method->transmembrane_helices_signal ) {				
				...
				if ( $feature->analysis->thump and $feature->analysis->thump->num_tmh ) {
					$method_score = $feature->analysis->thump->num_tmh;
				}				
				$output .= get_appris_annotations2(	$transcript_id,
													$gene_id,
													$external_id,
			...
			if ( defined $method->regions ) {
				foreach my $region (@{$method->regions}) {
					if ( defined $region->s_mean and defined $region->s_prob and 
						...
						$optional2->{'transcript_name'}	= $external_id;
						$optional2->{'version'}			= $version;
...
#				my ($method_type) = $GTF_CONSTANTS->{'crash'}->{'type'};
#				$output .= get_appris_annotations2(	$transcript_id,
#													$gene_id,
#													$external_id,
#													$feature,
