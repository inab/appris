...
sub fetch_by_region {
	...
	my ($entity) = $self->fetch_entity_by_region($chr, $start, $end, $get_analysis);
	return $entity;
}
...
sub fetch_entity_by_region {
	my ($self, $chr, $start, $end, $get_analysis) = @_;
...
sub fetch_by_position {
	...
	my ($entity) = $self->fetch_entity_by_position($chr, $start, $end, $get_analysis);
	return $entity;
}
...
sub fetch_entity_by_position {
	my ($self, $chr, $start, $end, $get_analysis, $type) = @_;
...
sub fetch_entity_by_xref_entry {
	...
	eval {
		...
		$xref_entry =~ /^ensg(.){11,13}/ or 
		$xref_entry =~ /^enst{11,13}/ or
		$xref_entry =~ /^ensp{11,13}/ or
		$xref_entry =~ /^ccds{6,7}/ or 
		$xref_entry =~ /^otthumt{11,13}/ or
		$xref_entry =~ /^otthumg{11,13}/
		) {			
		...
		elsif ( $xref_entry =~ /^ensg(\d){9,}/ or $xref_entry =~ /^enst(\d){9,}/ or $xref_entry =~ /^ensp(\d){9,}/ or
		$xref_entry =~ /^ccds(.){4,}/ or 
		$xref_entry =~ /^otthumt(.){9,}/ or $xref_entry =~ /^otthumg(.){9,}/
		) {
			# Check from the main 'entity' table
		...
		else 
		{
			if (
				...
				$xref_entry =~ /([ensg]{3})/ or $xref_entry =~ /([enst]{3})/ or $xref_entry =~ /([ensp]{3})/ or
				$xref_entry =~ /([otthumg]{3})/ or $xref_entry =~ /([otthumt]{3})/ or
				$xref_entry =~ /([ccds]{3})/ or
				$xref_entry =~ /g(\d){2,}/ or $xref_entry =~ /t(\d){2,}/ or $xref_entry =~ /p(\d){2,}/
			) {
...
sub fetch_analysis_by_stable_id {
	...
	if (defined $type and ($type eq 'firestar' or $type eq 'all')) {
		...
		eval {	
			my ($list) = $self->dbadaptor->query_firestar_residues(entity_id => $entity->{'entity_id'});
	...
	if (defined $type and ($type eq 'matador3d' or $type eq 'all')) {
		...
		eval {	
			my ($list) = $self->dbadaptor->query_matador3d_alignments(entity_id => $entity->{'entity_id'});
			foreach my $residue (@{$list}) {
	...
	if (defined $type and ($type eq 'spade' or $type eq 'all')) {
		...
		eval {
			my ($list) = $self->dbadaptor->query_spade_alignments(entity_id => $entity->{'entity_id'});
			foreach my $residue (@{$list}) {
				push(@{$regions},
	...
	if (defined $type and ($type eq 'crash' or $type eq 'all')) {
		...
		eval {
			my ($list) = $self->dbadaptor->query_crash_residues(entity_id => $entity->{'entity_id'});
			foreach my $residue (@{$list}) {
				push(@{$regions},
	...
	if (defined $type and ($type eq 'thump' or $type eq 'all')) {
		...
		eval {
			my ($list) = $self->dbadaptor->query_thump_helixes(entity_id => $entity->{'entity_id'});
			foreach my $residue (@{$list}) {
				push(@{$regions},
	...
	if (defined $type and ($type eq 'cexonic' or $type eq 'all')) {
		...
		my ($regions);		
		eval {	
			my ($list) = $self->dbadaptor->query_cexonic(entity_id => $entity->{'entity_id'});
			if (defined $list and scalar(@{$list}) > 0) {
				$method = $list->[0];
			}				
		};
		...
		eval {
			my ($list) = $self->dbadaptor->query_cexonic_residues(entity_id => $entity->{'entity_id'});
			foreach my $residue (@{$list}) {
				push(@{$regions},
		...
		if (defined $method) { # Create object
			eval {
				...
				(
					...
					-exon_conservation_signal			=> $method->{'exon_conservation_signal'},
					-transmembrane_helices_signal		=> $method->{'transmembrane_helices_signal'},
					-peptide_signal						=> $method->{'peptide_signal'},
					-mitochondrial_signal				=> $method->{'mitochondrial_signal'},
					-principal_isoform_signal			=> $method->{'principal_isoform_signal'},
					
					-functional_residues_rscore			=> $method->{'functional_residues_rscore'},						
					-homologous_structure_rscore		=> $method->{'homologous_structure_rscore'},
					-vertebrate_conservation_rscore		=> $method->{'vertebrate_conservation_rscore'},
					-domain_rscore						=> $method->{'domain_rscore'},						
					-unusual_evolution_rscore			=> $method->{'unusual_evolution_rscore'},						
					-exon_conservation_rscore			=> $method->{'exon_conservation_rscore'},		
					-transmembrane_helices_rscore		=> $method->{'transmembrane_helices_rscore'},						
					-peptide_mitochondrial_rscore		=> $method->{'peptide_mitochondrial_rscore'},					
					-principal_isoform_rscore			=> $method->{'principal_isoform_rscore'},
					...
					-exon_conservation_score			=> $method->{'exon_conservation_score'},
