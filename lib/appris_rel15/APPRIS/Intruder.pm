...
sub feed_gene_by_analysis {
	...
	if ($entity->analysis) {
		...
		# Insert CExonic analysis -----------------
		if (defined $type and ($type eq 'cexonic' or $type eq 'all') and $analysis->cexonic) {
			...
			if ($method->result) {
				...
				throw('No cexonic analysis');			
			}

			my($global_id);
			eval {
				$global_id = $self->dbadaptor->insert_cexonic(
								entity_id		=> $internal_entity_id,
								result			=> $result
				);
			};
			...
			if ( defined $method->regions ) {
				foreach my $region (@{$method->regions}) {
					if ( defined $region->start and defined $region->end and defined $region->strand ) {
						eval {
							my (%parameters) = (
											cexonic_id			=> $global_id,
											trans_start			=> $region->start,
											trans_end			=> $region->end,
											trans_strand		=> $region->strand						
							);

							my ($method_residues_id) = $self->dbadaptor->insert_cexonic_residues(%parameters);
						};
						throw('No cexonic residue analysis') if ($@);					
					}
					else {
						throw('No cexonic residue analysis');						
					}
				}
			}
		}
