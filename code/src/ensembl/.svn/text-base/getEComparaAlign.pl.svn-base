...
sub main()
{
	...
	$logger->info("-- -- load ensembl core registry\n");
	my ($e_core_registry) = 'Bio::EnsEMBL::Registry';
	eval {
		$e_core_registry->load_registry_from_db(%{$e_core_param});
	};
	return undef if $@;

	# Get cds data from input file
	$logger->info("-- get gtf data\n");
	my ($genes_data) = parse_gencode($data_file, $transcripts_file, $translations_file, $e_core_registry);	
...
sub get_align_by_trans($$$)
{
	...
	while ( my ($transcript_id, $cds_list) = each(%{$transc_align_list}) ) {
		...
		foreach my $cds (@{$cds_list}) {
			...
			my ($align_blocks) = get_ensembl_specie_align_from_coord($e_compara_registry, $chr, $start, $end, $strand);
			
			$logger->info("-- -- -- get_align_strings\n");
			get_align_strings($e_compara_registry, $align_blocks, $trans_align);				
		}
	...
	foreach my $gene (@{$genes_data}) {
		...
		if ( defined $gene->transcripts ) {
			...
			foreach my $transcript (@{$gene->transcripts})
			{
				...
				# BEGIN: FIXING A PROBLEM
				$chr =~ s/^M$/MT/;
				# END: FIXING A PROBLEM
				...
				if ( defined $atype ) {
					...
					# print align tree
					#$logger->info("-- -- -- get_ensembl_specie_aligntree_from_coord>$chr:$start-$end:$strand\n");
...
sub get_ensembl_specie_align_from_coord($$$$$)
{
	...
	my ($method_link_species_set) = $method_link_species_set_adaptor->fetch_by_method_link_type_species_set_name("EPO_LOW_COVERAGE", "mammals");
...
sub get_ensembl_specie_aligntree_from_coord($$$$$)
{
	...
	# Note: we obtain the last "align-tree"... Why? Because I THINK is the one that is the multiple alignment
	foreach my $genomic_align_tree (@{$genomic_align_trees}) {		
		if (UNIVERSAL::isa($genomic_align_tree, "Bio::EnsEMBL::Compara::GenomicAlignTree")) {			
			$align_tree_nh = $genomic_align_tree->newick_format('simple');
			$align_tree_nh =~ s/\[[+|-]+\]//mg; # remove strange characters (strand label)
			last;
		}
	}
	# We obtain the tree with more nodes: THIS NOT WORKS!!!
	#my ($num_nodes);
	#foreach my $genomic_align_tree (@{$genomic_align_trees}) {		
	#	if (UNIVERSAL::isa($genomic_align_tree, "Bio::EnsEMBL::Compara::GenomicAlignTree")) {			
	#		my ($tmp_align_tree_nh) = $genomic_align_tree->newick_format('simple');
	#		$tmp_align_tree_nh =~ s/\[[+|-]+\]//mg; # remove strange characters (strand label)
	#		my ($n_nodes) = get_num_tree_nodes($tmp_align_tree_nh);
	#		unless ( defined $num_nodes ) {
	#			$num_nodes = $n_nodes;
	#			$align_tree_nh = $tmp_align_tree_nh;
	#		}
	#		else {
...
sub get_align_strings($$\$)
{
	my ($registry, $align_blocks, $ref_report) = @_;
	my ($main_specie) = lc($species); $main_specie =~ s/^\s*//; $main_specie =~ s/\s*$//; $main_specie =~ s/\s/\_/;	
	my ($report);
	my ($loc_rep);
