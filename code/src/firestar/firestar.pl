...
$DEFALULT_FIRESTAR_CONFIG_FILE = $LOCAL_PWD.'/CONFIG_fire_var.ini';
$WSPACE_ID			= $cfg->val('APPRIS_PIPELINE', 'workspace');
$WSPACE_BASE		= $ENV{APPRIS_PROGRAMS_TMP_DIR}.'/'.$cfg->val('APPRIS_PIPELINE', 'workspace').'/'.$cfg->val('FIRESTAR_VARS', 'name').'/';
...
sub main()
{
	...
	$firestar_config_cont = $subs_template->($firestar_config_cont, 'APPRIS__PIPELINE__WORKSPACE', $WSPACE_ID);
	...
	foreach my $varname (keys %gene_vars)
	{
		...
		unless ( -e $firePredText_file and (-s $firePredText_file > 0) ) {
			...
			my ($cmd) = "$LOCAL_PWD/perl/firePredText.pl $varname $varname $seq 10 $firePredText_file 65 YES NO appris '' $firestar_config_file 2> $firePredText_log";
			$logger->debug("\n** script: $cmd\n");			
