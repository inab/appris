...
$WSPACE_BASE		= $ENV{APPRIS_PROGRAMS_TMP_DIR}.'/'.$cfg->val('APPRIS_PIPELINE', 'workspace').'/'.$cfg->val('INERTIA_VARS', 'name').'/';
$WSPACE_CACHE		= $ENV{APPRIS_PROGRAMS_TMP_DIR}.'/'.$cfg->val('APPRIS_PIPELINE', 'workspace').'/'.$cfg->val('CACHE_VARS', 'name').'/';
$WSPACE_DATA		= $ENV{APPRIS_PROGRAMS_TMP_DIR}.'/'.$cfg->val('APPRIS_PIPELINE', 'workspace').'/'.$cfg->val('INPUT_VARS', 'name').'/';
...
{	
	...
	foreach my $align ( @{$PROG_IN_ALIGNS} ) {
		$align_files = create_aligns($align);		
	}
	...
	foreach my $main_align_filename (@{$main_align_files}) {
		...
		if ( ($main_align_filename =~ /^(.*)$inalign_suffix/) and -e $main_align_file and (-s $main_align_file > 0) ) {
			...
			if ( -e $main_tree_file and (-s $main_tree_file > 0) ) {
				...
				$logger->error("no phylogenetic tree") if($@);
			}
		}
	}
	return $align_files;
...
sub _get_gene_list_from_gff($$)
{
	...
	foreach my $t ( keys(%{$all_t}) ) {
		#$transcript_codition .= ' $12=="\"'.$t.'\";" ||'; # for rel3c version
