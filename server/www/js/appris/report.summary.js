...
*/
function createAPPRISsumaryHTML(sSpecie,oReport)
{
	var oBody = new Array();
	...
	for (var i = 0; i < oReport.length; i++) {
		...
		if ( oLine.transcript_id !== undefined ) {
			//var sTranscriptId = oLine.transcript_id;
			var sTransIdLink = APPRIS.ENSEMBL_URL + '/'+ sSpecie + '/Transcript/Summary?db=core;t='+ oLine.transcript_id;
...
*/
function getTranscriptInfoFromAPPRIS(sQueryId, sSpecie)
{
	...
	$.ajax({
		...
		success: function(data) {		
			if (data == null) { // ERROR message
				...
				// Add report to table
				var sReportHTML = createAPPRISsumaryHTML(sSpecie,data);
