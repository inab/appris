			...
			if (sIdDBLink && sNamespaceDBLink && sLabelDBLink) {
				if (sNamespaceDBLink == "External_Id") {
					...
					if (sNamespaceDBLink == "Ensembl_Gene_Id") {
						var sReportLink = "javascript: window.location='report.html?id="+sIdDBLink+"&namespace="+sNamespaceDBLink+"&specie="+sSpecie+"'";
...
*/
function runReportPipeline(sQueryId, sNamespace, sSpecie)
{
	var oXML = $.ajax({
		type: 'GET',
		...
		success: function(data) {		
			if (data == null) { // ERROR message
				...
				if (printSearchInfos(data)) {
					...
					getTranscriptInfoFromAPPRIS(sQueryId,sSpecie);
					getAPPRISInfo(sQueryId,sSpecie,sQueryIdList);
					getSequence(sQueryId,sSpecie,'aa');
					getUCSCGenomeBrowser(sQueryId,sSpecie);
					getProteoUCSCGenomeBrowser(sQueryId, sSpecie);
					getRNASeqUCSCGenomeBrowser(sQueryId, sSpecie);
				}
				else {
					stopStatusPanels(); // No Entries
				}
