...
function printSequenceInfo(aSeqReportList, sSpecie)
{
	var sResponse =	'';	
	for (var i = 0; i < aSeqReportList.length; i++) {
		...
		sResponse += "<tr id='"+sId+"' class='title'>"+
						"<td class='title'>"+
							"<input class='sequence_id' type='checkbox' value='"+sSpecie+"_"+sId+"'/>"+
						"</td>"+
						"<td class='title'>"+
...
{	
	...
	// Sort by id, case-insensitive, A-Z
	aSeqReportList.sort(sort_by('id', true, function(a){return a.toUpperCase()}));
	
	// Sort by legnth
	// aSeqReportList.sort(sort_by('length', true, parseInt));
	
	return aSeqReportList;
}

// Get sequences from given query
function getSequence(sQueryId,sSpecie,sType)
{
	printStatusMessage('#sequence_panel',true,'Loading...');

	$.ajax({
		type: 'GET',
		url: APPRIS.RESTWS+'sequence/id/'+sSpecie+'/'+sQueryId,
		data: {
			'type': sType,
			'format': 'fasta'
		},
		success: function(text,textStatus) {
			...
			if (text != '') {
				...
				var sReportHTML = printSequenceInfo(aSeqReportList, sSpecie);
				
				$('#sequence_panel .report').html(sReportHTML);
				
				confSeqMenus(aSeqReportList,sSpecie);
			}
...
function getImgSeq(sQueryId, sSpecie)
{
	...
	$.ajax({
		type: 'GET',
		url: APPRIS.RESTWS+'img_seq/id/'+sSpecie+'/'+sQueryId,
		dataType: 'xml',
...
function confSeqMenus(aSeqReportList,sSpecie)
{
	for (var i = 0; i < aSeqReportList.length; i++) {
		var oSeqReport = aSeqReportList[i];
		var sId = oSeqReport.id;
			confSeqMenu(sId);
			confAllMenu(sId,sSpecie);
	}
}
...
function confAllMenu(sQueryId,sSpecie)
{
