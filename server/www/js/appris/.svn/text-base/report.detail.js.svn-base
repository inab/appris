...
* @param  {String} sSpecie
* @return {String} sResponse HTML response
*/
function printAPPRISInfo(sQueryId,sSpecie,oReport)
{
	var sExportData = '';
	...
	if (oReport) {
		...
		for (var sMethodType in APPRIS.METHOD_LABELS) {
			if ( sMethodType != APPRIS.APPRIS_TYPE ) { // no print the final annotation of appris
				...
				var sMethod = APPRIS.METHOD_NAMES[sMethodType];
				var sMethodLink = APPRIS.RESTWS_URL+sMethod+'/id/'+sSpecie+'/'+sQueryId;
			 	sResponse += "<th id='"+sMethodType+"'>"+
								"<div class='label'>"+sMethodLabel+
									"<span class='pop_external_ref' title='Result in detail' value='"+sMethodLink+"'>&nbsp;&nbsp;&nbsp;</span>"+
								"</div>"+
							"</th>";
			...
			if ( sType != APPRIS.APPRIS_TYPE ) { // no print the final annotation of appris
				...
				if ( (sValueLabel != '-') && (sValueLabel != '0') ) {
					...
					if ( !(sMethod == 'vcrash' && sValue == APPRIS.NO_LABEL) ) {
						var sMethodLink = APPRIS.RESTWS_URL+sMethod+'/id/'+sSpecie+'/'+sTranscriptId;
						sValueLabel = "<div class='pop_external_ref' title='Simplified result' value='"+sMethodLink+"'>"+ sValueLabel +"</div>";
					}
				}
			...
			if (sSource != '' && sSource == 'APPRIS' && sType != '' && sScore != '') {				
				switch (sType) {
					...
					case APPRIS.SPADE_TYPE:
							if(sScore != '.') {								
								oTranscriptReportList[sTranscriptId][APPRIS.SPADE_TYPE] = sScore;
								bHasResults = true;
							}					
						break;						
...
* @param {String} sSpecie specie name 
* @param {String} sQueryIdList List of all protein coding identifiers of gene
*/
function getAPPRISInfo(sQueryId,sSpecie,sQueryIdList)
{
	printStatusMessage('#appris_panel',true,'Loading...');

	$.ajax({
		type: 'GET',
		url: APPRIS.RESTWS+'export/id/'+sSpecie+'/'+sQueryId,		
		data: {
			'format': 'json'
		},
		dataType: 'json',
		success: function(data) {		
			if (data == null) { // ERROR message
				...
				var sReportHTML = printAPPRISInfo(sQueryId,sSpecie,oAPPRISReports);
