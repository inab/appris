...
function getUCSCGenomeBrowser(sQueryId)
{
		...
		success: function(data) {
			if (data == null) {
				...
				if (data.error) {
					...
					$('#ucsc_appris_panel table.ucsc_img_head').css({ 'background-image': "url('"+sUCSC_PROXY_IMG_BG+"')" });
		...
		success: function(data) {
			...
			if (data == null) {
				...
				if (data.error) {
					...
					$('#ucsc_appris_panel table.ucsc_img_data').css({ 'background-image': "url('"+sUCSC_PROXY_IMG_BG+"')" });					
					...
					var sAPPRIS = APPRIS.RESTWS_URL + "export/id/"+sQueryId+'?'+
																	"format=bed"+'&'+
