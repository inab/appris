...
sub main()
{
	...
	if (defined $format and ($format ne '')) {
		$format = lc($format);
		if (($format ne '') and ( ($format eq 'gtf') or ($format eq 'bed') or ($format eq 'json') ) ) {
			$FORMAT = $format;	
		}
		else {
			print_http_response(400);
...
#print STDERR "\nINPUT:\n".Dumper($input)."\n";
	...
	my ($features);
	if ( $type eq 'id' ) {
		$features = get_report_by_stable_id($input);
	}
	elsif ( $type eq 'name' ) {
		$features = get_report_by_xref_entry($input);
	}
	elsif ( $type eq 'position' ) { 
		$features = get_report_by_position($input);		
	}
	else {
		print_http_response(400);
	}
