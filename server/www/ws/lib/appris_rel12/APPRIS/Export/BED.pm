...
#print DATA_7 "\nRES_DAMAGED: $transcript_id\n".Dumper($res_damaged_domains)."\n";
				...
				_aux_get_spade_annotations('domain',
											...
											$res_domains,
											$ref_output);
				_aux_get_spade_annotations2('damaged_domain',
											$transcript_id,
											$feature,
											$res_damaged_domains,
											$ref_output);
...
sub _overlap_aligments {
	my ($aux_res_list) = @_;
	my ($res_list);
	
	my (@range_list);
	foreach my $res (@{$aux_res_list}) {
		my $r2 = Bio::Range->new(-start=>$res->start, -end=>$res->end, -strand=>$res->strand);
		push(@range_list, $r2);
	}
	
	my (@unions) = Bio::Range->unions(@range_list);
	if ( scalar(@unions) > 0 ) {
		my ($start) = $unions[0]->start;
		my ($end) = $unions[0]->end;
		my ($strand) = $unions[0]->strand;
		push(@{$res_list}, $unions[0]);
	}
	else {
		$res_list = $aux_res_list;
	}
	return $res_list;	
}

sub _aux_get_spade_annotations2 {
	my ($type, $transcript_id, $feature, $aux_res_list, $ref_output) = @_;
...
#print DATA_7 "RES_LIST: $transcript_id:\n".Dumper($res_list)."\n";
		...
		# get overlap alignments
		$res_list = _overlap_aligments($res_list);
#print DATA_7 "OVERLAP_RES_LIST: $transcript_id:\n".Dumper($res_list)."\n";		
