...
=head2 new
  ...
  Arg [-j_params]: (optional)
        string - the parameters of queue
...
sub new {
	...
	# optional paramas
	$self->j_params($j_params) if (defined $j_params);
...
=head2 j_params
  ...
  Arg [1]    : (optional) String - the optional parameters for job
  Example    : $analysis->j_params($params);
  Description: Getter/setter for the parameters of job 
  Returntype : String
  Exceptions : none
  Caller     : general
  Status     : Stable
...
sub j_params {
	...
	if (@_) {
		$self->{'j_params'} = shift;
		$self->_subs_template('__JOB__PARAMETERS__', $self->{'j_params'});
	}
	return $self->{'j_params'};
}
