...
# Required arguments
unless ( defined $numlist and defined $seqfile and defined $outfile ){
	print `perldoc $0`;
	exit 1;
}
...
{	
	...
	my ($lenseqreport) = _parse_len_seq($seqfile);
...
create_genelist
	...
	--seq=pardinus23A/features/20058.LYPA.23A.transl.fa

	--outfile=pardinus23A/features/20058.LYPA.23A.genelist.txt

	--loglevel=INFO
	
	--logappend
	
	--logpath=pardinus23A/logs/
