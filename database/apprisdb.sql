...
--
-- Table structure for table cexonic
-- Includes the two alignmets reported by cexonic method
--
CREATE TABLE cexonic (
  cexonic_id INT(11) unsigned NOT NULL auto_increment,
  entity_id INT(11) unsigned NOT NULL,
  result LONGTEXT DEFAULT NULL,
  num_introns INT(5) unsigned DEFAULT NULL,
  first_specie_num_exons INT(5) unsigned DEFAULT NULL,
  second_specie_num_exons INT(5) unsigned DEFAULT NULL,
  conservation_exon ENUM('NO','YES','UNKNOWN') DEFAULT NULL,
  CONSTRAINT fk_cexonic_entity FOREIGN KEY (entity_id) REFERENCES entity (entity_id) ON DELETE CASCADE ON UPDATE CASCADE,
  PRIMARY KEY (cexonic_id),
  UNIQUE KEY unique_key_cexonic_cexonic_id (cexonic_id),
  UNIQUE KEY key_cexonic_entity_id (entity_id)
) ENGINE=InnoDB CHARSET=utf8;

--
-- Table structure for table cexonic_residues
--
CREATE TABLE cexonic_residues (
  cexonic_residues_id INT(11) unsigned NOT NULL auto_increment,
  cexonic_id INT(11) unsigned NOT NULL,
  trans_start INT(20) unsigned NOT NULL,
  trans_end INT(20) unsigned NOT NULL,
  trans_strand ENUM('.','+','-') NOT NULL,
  CONSTRAINT fk_cexonic_residues_cexonic FOREIGN KEY (cexonic_id) REFERENCES cexonic (cexonic_id) ON DELETE CASCADE ON UPDATE CASCADE,
  PRIMARY KEY (cexonic_residues_id),
  UNIQUE KEY unique_key_cexonic_residues_cexonic_residues_id (cexonic_residues_id),
  INDEX index_cexonic_residues_cexonic_residues_id_cexonic_id (cexonic_residues_id,cexonic_id)
) ENGINE=InnoDB CHARSET=utf8;
...
CREATE TABLE appris (
  ...
  exon_conservation_signal ENUM('.','NO','YES','UNKNOWN') DEFAULT '.', -- CExonic
  ...
  functional_residues_rscore CHAR(1) DEFAULT NULL, -- firestar
  homologous_structure_rscore CHAR(1) DEFAULT NULL, -- Matador3D
  vertebrate_conservation_rscore CHAR(1) DEFAULT NULL, -- CORSAIR
  domain_rscore CHAR(1) DEFAULT NULL, -- SPADE
  unusual_evolution_rscore CHAR(1) DEFAULT NULL, -- INERTIA
  exon_conservation_rscore CHAR(1) DEFAULT NULL, -- CExonic
  transmembrane_helices_rscore CHAR(1) DEFAULT NULL, -- THUMP
  peptide_mitochondrial_rscore CHAR(1) DEFAULT NULL, -- CRASH
  principal_isoform_rscore VARCHAR(3) DEFAULT NULL, -- APPRIS
  reliability VARCHAR(10) DEFAULT NULL,
  ...
  exon_conservation_score VARCHAR(10) DEFAULT NULL, -- CExonic
